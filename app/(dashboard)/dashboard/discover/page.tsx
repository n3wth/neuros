  ...
  try {
    ...
    if (authError) {
      logger.error('Dashboard auth error', {
        error: authError,
        action: 'dashboard_discover_load'
      })
      redirect('/signin')
    }
    ...
    if (!user) {
      logger.info('Dashboard redirect - no user', {
        action: 'dashboard_discover_redirect'
      })
      redirect('/signin')
    }

    // Log successful auth
    logger.info('Dashboard Discover loaded', {
      userId: user.id,
      action: 'dashboard_discover_load_success'
    })
    ...
    if (isMobile) {
      return <MobileOptimizedDashboard user={serializedUser} initialViewMode="discover" />
    }

    // Use full dashboard for desktop
    return <FullLearningDashboard user={serializedUser} initialViewMode="discover" />
  } catch (error) {
    ...
    // Log actual errors
    logger.error('Dashboard Discover page error', {
      error: err,
      action: 'dashboard_discover_error'
    })
