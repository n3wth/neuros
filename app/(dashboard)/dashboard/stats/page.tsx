  ...
  try {
    ...
    if (!user) {
      logger.info('Dashboard redirect - no user', {
        action: 'dashboard_stats_redirect'
      })
      redirect('/signin')
    }
    ...
    logger.info('Dashboard Stats loaded', {
      userId: user.id,
      action: 'dashboard_stats_load_success'
    })
    ...
    if (isMobile) {
      return <MobileOptimizedDashboard user={serializedUser} initialViewMode="stats" />
    }

    // Use full dashboard for desktop
    return <FullLearningDashboard user={serializedUser} initialViewMode="stats" />
  } catch (error) {
    ...
    // Log actual errors
    logger.error('Dashboard Stats page error', {
      error: err,
