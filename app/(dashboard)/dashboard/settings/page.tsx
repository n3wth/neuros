  ...
  try {
    ...
    if (authError) {
      logger.error('Dashboard auth error', {
        error: authError,
        action: 'dashboard_settings_load'
      })
      redirect('/signin')
    }
    ...
    if (!user) {
      logger.info('Dashboard redirect - no user', {
        action: 'dashboard_settings_redirect'
      })
      redirect('/signin')
    }
    ...
    logger.info('Dashboard Settings loaded', {
      userId: user.id,
      action: 'dashboard_settings_load_success'
    })
    ...
    if (isMobile) {
      return <MobileOptimizedDashboard user={serializedUser} initialViewMode="settings" />
    }

    // Use full dashboard for desktop
    return <FullLearningDashboard user={serializedUser} initialViewMode="settings" />
  } catch (error) {
    ...
    // Log actual errors
    logger.error('Dashboard Settings page error', {
      error: err,
      action: 'dashboard_settings_error'
    })
